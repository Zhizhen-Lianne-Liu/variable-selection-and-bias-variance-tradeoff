#contains ammended greedysubset() from q4
import numpy as np
import scipy.stats
...
def improvedfit(bestRSS, bestRSS_prev, N, d):
    F_statistic = (bestRSS_prev - bestRSS)/(bestRSS/(N-d-1))

    alpha = 0.95
    p_value = scipy.stats.f.cdf(F_statistic, 1, N-d-1)
...
def greedysubset(T):
    ...
    for i in range (1, p+1):
        ...
        bestRSS = RSS[index_min]  #best RSS of previous model
        if i >= 2:
            if improvedfit(bestRSS, bestRSS_prev, Ntr, i) == False:
